...
long long int sum_simd(int vals[NUM_ELEMS]) {
  ...
  for (unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
    ...
    for (unsigned int i = 0; i < NUM_ELEMS / 4 * 4; i += 4) {
      ...
      // check vals >= 128
      __m128i m = _mm_cmpgt_epi32(vec, _127);
      sum_v = _mm_add_epi32(sum_v, _mm_and_si128(vec, m));
    }

    int tmp_vals[4];
    _mm_storeu_si128((__m128i*)tmp_vals, sum_v);
...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {
  ...
  for (unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
    ...
    for (unsigned int i = 0; i < NUM_ELEMS / 16 * 16; i += 16) {
      ...
      vec = _mm_load_si128((__m128i*)(vals + i + 4));
      m = _mm_cmpgt_epi32(vec, _127);
      sum_v = _mm_add_epi32(sum_v, _mm_and_si128(vec, m));

      vec = _mm_load_si128((__m128i*)(vals + i + 8));
      m = _mm_cmpgt_epi32(vec, _127);
      sum_v = _mm_add_epi32(sum_v, _mm_and_si128(vec, m));

      vec = _mm_load_si128((__m128i*)(vals + i + 12));
      m = _mm_cmpgt_epi32(vec, _127);
      sum_v = _mm_add_epi32(sum_v, _mm_and_si128(vec, m));
    }
