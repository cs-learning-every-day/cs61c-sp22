...
void v_add_optimized_chunks(double* x, double* y, double* z) {
  ...
  int thread_idx = thread_num * chunk_size;
#pragma omp parallel
  {
    int tmp_idx;

    for (int i = 0; i < chunk_size; i++) {
      tmp_idx = thread_idx + i;
      z[tmp_idx] = x[tmp_idx] + y[tmp_idx];
    }

    if ((thread_num == num_threads - 1) && (ARRAY_SIZE % num_threads != 0)) {
...
#pragma omp parallel
  {
    double tmp_sum = 0.0;
#pragma omp for
    for (int i = 0; i < arr_size; i++) tmp_sum += x[i] * y[i];
#pragma omp critical
    global_sum += tmp_sum;
  }
  return global_sum;
