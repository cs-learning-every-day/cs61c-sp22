...
int allocate_matrix(matrix **mat, int rows, int cols) {
  ...
  matrix *nmatrix = (matrix *)malloc(sizeof(matrix));
  if (nmatrix == NULL) {
    return -2;
  }

  nmatrix->data = (double *)calloc(rows * cols, sizeof(double));
  if (nmatrix->data == NULL) {
    return -2;
  }
  ...
  for (int i = 0; i < result->rows; i++) {
    for (int j = 0; j < result->cols; j++) {
      ...
      for (int idx = 0; idx < result->cols; idx++) {
        val += get(mat1, i, idx) * get(mat2, idx, j);
      }
      set(result, i, j, val);
    }
  }
...
int pow_matrix(matrix *result, matrix *mat, int pow) {
  ...
  allocate_matrix(&zero_matrix, result->rows, result->cols);
  fill_matrix(zero_matrix, 0);
  add_matrix(result, mat, zero_matrix);
  if (pow == 1) {
    return 0;
  }
  ...
  allocate_matrix(&tmp_matrix, result->rows, result->cols);
  for (int i = 0; i < pow; i++) {
    mul_matrix(tmp_matrix, result, mat);
    add_matrix(result, zero_matrix, tmp_matrix);
  }
